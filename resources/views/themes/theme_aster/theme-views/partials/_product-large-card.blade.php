...
<div class="product border rounded text-center d-flex flex-column gap-10 ov-hidden cursor-pointer product-card-shadow scale-image-hover-effect"
    ...
    <div class="product__top" style="--width: 100%;">
        ...
        @if(isset($product->flash_deal_status) && $product->flash_deal_status)
            <div class="product__power-badge">
                <img src="{{ theme_asset('assets/img/svg/power.svg') }}" alt=""
                     class="svg text-white" loading="lazy">
            </div>
        @endif
        ...
        <div class="product__thumbnail">
            <img src="{{\App\CPU\ProductManager::product_image_path('thumbnail')}}/{{$product['thumbnail']}}"
                 ...
                 class="img-fit card-product-image dark-support rounded prod-imag2" alt="">
        </div>
    ...
    <!-- Product Summery -->
    <div class="d-flex align-items-center justify-content-between responsive-sides-padding" >
        <div class="text-muted font-size-10-mobile">
            @if($product->added_by=='seller')
                {{ isset($product->seller->shop->name) ? \Illuminate\Support\Str::limit($product->seller->shop->name, 10) : '' }}
            @elseif($product->added_by=='admin')
                {{$web_config['name']->value}}
            @endif
        </div>
        <div class="d-flex gap-1 align-items-center">
            <div class="star-rating text-gold font-size-10-mobile">
                @for ($i = 1; $i <= 5; $i++)
    ...
    <div class="product__summary d-flex flex-column gap-1 pb-2 cursor-pointer">
    
        <h6 class="product__title text-truncate text-start responsive-sides-padding">
            {{ \Illuminate\Support\Str::limit($product['name'], 25) }}
        </h6>

        <div style="text-align: unset;" class="product__price d-flex align-items-center p-2 responsive-sides-padding pb-1 justify-content-end flex-wrap column-gap-2">
            @if($product->discount > 0)
