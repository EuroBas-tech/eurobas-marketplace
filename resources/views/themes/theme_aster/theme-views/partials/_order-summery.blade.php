        ...
        <div class="card-body px-sm-4 d-flex flex-column gap-2">
            ...
            @if($cart->count() > 0)
                @foreach($cart as $key => $cartItem)
                    ...
                    @php($total_tax+=$cartItem['tax_model']=='exclude' ? ($cartItem['tax']*$cartItem['quantity']):$cartItem['tax'])
            ...
            <div class="d-flex mb-3">
                <h4>{{ translate('Order_Summary') }}</h4>
            </div>
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>{{ translate('Item_Price') }}</div>
                <div class="fw-bold currency-font" >{{\App\CPU\Helpers::currency_converter($product_price_total)}}</div>
            </div>
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>{{ translate('Product_Discount') }}</div>
                <div class="fw-bold currency-font" >{{\App\CPU\Helpers::currency_converter($total_discount_on_product)}}</div>
            </div>
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                ...
                <div class="fw-bold currency-font" >{{\App\CPU\Helpers::currency_converter($product_price_total - $total_discount_on_product)}}</div>
            </div>
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>{{ translate('tax') }}</div>
                <div class="fw-bold currency-font" >{{\App\CPU\Helpers::currency_converter($total_tax)}}</div>
            </div>
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>{{ translate('shipping') }}</div>
                <div class="text-primary fw-bold currency-font">{{\App\CPU\Helpers::currency_converter($total_shipping_cost)}}</div>
            </div>
            ...
            @if(auth('customer')->check() && session()->has('coupon_discount'))
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                    ...
                    <div class="text-primary fw-bold currency-font">- {{\App\CPU\Helpers::currency_converter($coupon_discount+$order_wise_shipping_discount)}}</div>
                </div>
            @endif
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                ...
                <!--<h2 class="text-primary currency-font">{{\App\CPU\Helpers::currency_converter($product_price_total+$total_tax+$total_shipping_cost-$coupon_dis-$total_discount_on_product-$order_wise_shipping_discount)}}</h2>-->
                <h2 class="text-primary currency-font">{{\App\CPU\Helpers::currency_converter($product_price_total+($cartItem['tax_model']=='exclude' ? $total_tax : 0)+$total_shipping_cost-$coupon_dis-$total_discount_on_product-$order_wise_shipping_discount)}}</h2>
