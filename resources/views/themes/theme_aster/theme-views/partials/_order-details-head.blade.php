<div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
    <div class="d-flex align-items-center flex-wrap gap-2 justify-content-between flex-grow-1 d-lg-block">
        <h5 class="mb-1">{{translate('Order')}} #{{$order['id']}} </h5>
        <p class="fs-12">{{date('d M, Y h:i A',strtotime($order->created_at))}} ({{$order->created_at->diffForHumans()}})</p>
    </div>

    <div class="">
        <div class="d-none d-lg-flex gap-3 justify-content-end mb-3">
            @if(($order->order_status=='delivered' || $order->order_status=='delivery_confirmed') &&  $order->order_type == 'default_type')
                <a href="javascript:" onclick="order_again({{ $order->id }})" class="btn btn-primary rounded p-1 px-3">{{ translate('reorder') }}</a>
            @endif
        </div>
        ...
        <div class="d-none d-lg-flex gap-3 align-items-center mt-2 justify-content-between">
            ...
            @if($order['order_status']=='failed' || $order['order_status']=='canceled')
                <span class="badge bg-danger rounded p-1 px-2">
                    {{translate($order['order_status'] =='failed' ? 'Failed To Deliver' : $order['order_status'])}}
                </span>
            @elseif($order['order_status']=='confirmed' || $order['order_status']=='processing' || $order['order_status']=='delivered' || $order['order_status']=='delivery_confirmed' )
                <span class="badge bg-success rounded p-1 px-2">
                    {{translate($order['order_status']=='processing' ? 'packaging' : $order['order_status'])}}
                </span>
            @else
                <span class="badge bg-info rounded p-1 px-2">
        ...
        <div class="d-none d-lg-flex gap-3 align-items-center mt-2 justify-content-between">
            ...
            <div class="{{ $order['payment_status']=='unpaid' ? 'text-danger':'text-success' }} fw-bold"> {{ translate($order['payment_status']) }}</div>
        </div>
...
<div class="mt-4">
    <nav>
        <div class="nav nav-nowrap gap-3 gap-xl-4 nav--tabs hide-scrollbar">
            ...
            <a href="{{ route('account-order-details-delivery-man-info', ['id'=>$order->id]) }}" class="{{Request::is('account-order-details-delivery-man-info')  ? 'active' :''}}" >{{translate('Shipping_Info')}}</a>
