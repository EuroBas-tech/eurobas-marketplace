<div class="dropdown">
    ...
    <ul class="dropdown-menu dropdown-menu-end scrollY-60" style="--bs-dropdown-min-width: 22.5rem">
            ...
            <div class="media gap-3">
                ...
                <div class="media-body">
                    <h6 class="mb-2 {{ $product->status == 0?'blur-section':'' }}">
                        <a href="{{ $product->status == 1 ? route('product',$cartItem['slug']) : 'javascript:'}}">{{Str::limit($cartItem['name'],50)}}</a>
                    </h6>

                    <div class="fs-12 pb-2"><span class="cart_quantity_{{ $cartItem['id'] }}">{{$cartItem['quantity']}}</span> × {{\App\CPU\Helpers::currency_converter(($cartItem['price']-$cartItem['discount']))}}</div>
                    ...
                    <div class="d-flex gap-3 justify-content-between align-items-end">
                        ...
                        <div class="d-flex flex-column gap-1 {{ $product->status == 0?'blur-section':'' }}">
                            <div class="product__price d-flex flex-wrap gap-2">
                                <del class="product__old-price quantity_price_of_{{ $cartItem['id'] }} currency-font">{{\App\CPU\Helpers::currency_converter($cartItem['price']*$cartItem['quantity'])}}</del>
                                <ins class="product__new-price discount_price_of_{{ $cartItem['id'] }} currency-font">{{\App\CPU\Helpers::currency_converter(($cartItem['price']-$cartItem['discount'])*(int)$cartItem['quantity'])}}</ins>
                            </div>
                        </div>
        ...
        <li class="separator-dashed-border" >
            <div class="flex-between-gap-3 py-2 pb-3">
