...
@endphp
<div class="container">
    <h4 class="fs-30 mb-3">{{ translate('Cart_List') }}</h4>
    <form action="#">
        <div class="row gy-3">
            <div class="col-lg-8">
                    ...
                    <div class="card-body">
                        ...
                        @if( $cart->count() > 0)
                            <div class="d-flex justify-content-center mb-30">
                                <ul class="cart-step-list">
                                    <li class="current cursor-pointer fs-20" onclick="location.href='{{route('shop-cart')}}'"><span><i class="bi bi-check2"></i></span> {{ translate('cart') }}</li>
                                    <li class=" cursor-pointer fs-20" onclick="location.href='{{ route('checkout-details') }}'"><span><i class="bi bi-check2"></i></span> {{ translate('Shopping_Details') }}</li>
                                    <li class=" fs-20" ><span><i class="bi bi-check2"></i></span> {{ translate('payment') }}</li>
                                </ul>
                            </div>
                        @endif
                        ...
                        @foreach($cart as $group_key=>$group)
                            ...
                            @foreach($group as $cart_key=>$cartItem)
                                ...
                                @if($cart_key==0)
                                    ...
                                    <div class="bg-primary-light rounded dashed-border py-3 px-2 px-sm-3 mb-3 mb-sm-4">
                                        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <span class="text-white" class="ps-2 text-danger pulse-button" data-bs-toggle="tooltip" data-bs-placement="right"
                            ...
                            <div class="table-responsive d-none d-sm-block rounded">
                                @php
                                    $physical_product = false;
                                    foreach ($group as $row) {
                                        if ($row->product_type == 'physical') {
                                ...
                                <table class="table align-middle rounded">
                                    <thead style="background: #ebe6e6 !important;" class="table-light rounded">
                                    <tr>
                                        <th class="border-0 font-size-13">{{ translate('product_details') }}</th>
                                        <th class="border-0 font-size-13 text-end">{{ translate('unit_price') }}</th>
                                        <th class="border-0 font-size-13 text-end">{{ translate('discount') }}</th>
                                        <th class="border-0 font-size-13 text-end">{{ translate('total') }}</th>
                                        @if ( $shipping_type != 'order_wise')
                                            <th class="border-0 font-size-13 text-end">{{ translate('shipping_cost') }} </th>
                                        @endif
                                        <th class="border-0 font-size-13 text-center">{{ translate('qty') }}</th>
                                        <th class="border-0 font-size-13 text-center"></th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($group as $cart_key=>$cartItem)
                                    ...
                                    @php($product = $cartItem->all_product)
                                        ...
                                        <tr>
                                                ...
                                                <div class="media align-items-center gap-3">
                                                    ...
                                                    <div class="media-body d-flex gap-1 flex-column {{ $product->status == 0?'blur-section':'' }}">
                                                        ...
                                                        @foreach(json_decode($cartItem['variations'],true) as $key1 =>$variation)
                                                            @if($key1 == "color")
                                                                <div class="fs-12 d-flex align-items-center gap-2"><span>{{$key1}} : </span><span class="show-color-in-card" style="background:{{$variation}};" ></span></div>
                                                            @else
                                            ...
                                            <td class="text-center">
                                                @if ($product->status == 1)
                                                    <div class="quantity quantity--style-two d-inline-flex">
                                                        <span class="quantity__minus " onclick="updateCartQuantityList('{{ $product->minimum_order_qty }}', '{{$cartItem['id']}}', '-1', 'minus')">
                                            ...
                                            <td class="text-center" >
                                                <span role="button" onclick="removeCart('{{$cartItem['id']}}')">
                                                    <i class="bi bi-trash3-fill text-danger fs-25"></i>
                                                </span>
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        @if( $cart->count() == 0)
                            <div class="d-flex justify-content-center align-items-center">
                                <h2 class="text-danger text-capitalize">{{ translate('cart_empty') }}</h2>
                            </div>
                            <div class="text-center" >
                                <i class="bi bi-cart-x very-large-font-size text-danger"></i>
                            </div>
                        @endif
                                ...
                                <div class="form-group mt-3">
                                    <div class="row">
                                        <div class="col-12">
                                            ...
                                            <textarea class="form-control w-100" rows="5" placeholder="Enter a note to seller..." id="order_note" name="order_note">{{ session('order_note')}}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </form>
...
<script>
    ...
    function updateCartCommon(minimum_order_qty, key, e, quantity, ex_quantity) {
        if(quantity < minimum_order_qty && e == 'minus' ) {
