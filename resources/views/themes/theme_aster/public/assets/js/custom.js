...
document.querySelector('.closemob').addEventListener('click', function() {
    document.getElementById('popupmob').style.display = 'none';
...
document.getElementById('searchInputmob').addEventListener('input', function() {
    var searchQuery = this.value.toLowerCase().trim();
    var countries = document.querySelectorAll('.country-item-mob');
    countries.forEach(function(country) {
        var text = country.textContent.toLowerCase().replace(/\s+/g, ' ').trim();
        var match = text.includes(searchQuery);
        country.style.display = match ? 'block' : 'none';
...
document.getElementById('searchInputmob').addEventListener('focus', function() {
    var selects = document.querySelectorAll('.custom-select-mob');
...
document.getElementById('currencySearchInputmob').addEventListener('input', function() {
    var searchQuery = this.value.toLowerCase().trim();
    var countries = document.querySelectorAll('.currency-item-mob');
    countries.forEach(function(country) {
        var text = country.textContent.toLowerCase().replace(/\s+/g, ' ').trim();
        var match = text.includes(searchQuery);
        country.style.display = match ? 'block' : 'none';
...
document.getElementById('currencySearchInputmob').addEventListener('focus', function() {
    document.querySelector('.custom-select-mob-currency').style.display = 'block';
});


// إخفاء القائمة عند اختيار عنصر
document.querySelectorAll('.country-item-mob').forEach(function(item) {
    item.addEventListener('click', function() {
        document.getElementById('searchInputmob').value = this.textContent.trim();
        document.querySelector('.custom-select-mob').style.display = 'none';
        // أضف هنا الكود للتعامل مع القيمة المختارة
    });
});

// إخفاء القائمة عند اختيار عنصر
document.querySelectorAll('.currency-item-mob').forEach(function(item) {
    item.addEventListener('click', function() {
        document.getElementById('currencySearchInputmob').value = this.textContent.trim();
        document.querySelector('.custom-select-mob').style.display = 'none';
        // أضف هنا الكود للتعامل مع القيمة المختارة
    });
});
...
function removeWishlist(product_id, url) {
    ...
    $.ajax({
        ...
        success: function (data) {
            Swal.fire({
                icon: "success",
                title: "Ad removed from wishlist with success",
