    ...
    <script>
        ...
        setInterval(function() {
            $.get({
                ...
                success: function(response) {
                    let data = response.data;
                    if (data.canceled_orders > 0) {
                        playAudio();
                        $('#canceled-orders-popup-modal').appendTo("body").modal('show');
                    }
                },
            });
        }, 10000);
        ...
        setInterval(function() {
            $.get({
                ...
                success: function(response) {
                    let data = response.data;
                    if (data.balance_transactions > 0) {
                        playAudio();
                        $('#balance-transactions-popup-modal').appendTo("body").modal('show');
                    }
                },
            });
        }, 10000);
        ...
        setInterval(function() {
            $.get({
                ...
                success: function(response) {
                    let data = response.data;
                    if (data.refund_requests > 0) {
                        playAudio();
                        $('#refund_request-popup-modal').appendTo("body").modal('show');
                    }
                },
            });
        }, 10000);
        ...
        function check_refund_request() {
            location.href = "{{route('seller.refund.list', 'pending')}}";
        }
        function check_canceled_orders() {
            location.href = "{{ route('seller.orders.list', ['status' => 'canceled']) }}";
        }
    </script>
    <script>
