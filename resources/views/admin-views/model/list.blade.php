...
@section('title', translate('model_List'))
                                ...
                                <tbody>
                                    @foreach($models as $k=>$m)
                                        <tr>
                                            ...
                                            <td>
                                                <form action="{{route('admin.model.status-update')}}" method="post" id="model_status{{$m['id']}}_form" class="model_status_form">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{$m['id']}}">
                                                    <label class="switcher mx-auto">
                                                        <input type="checkbox" class="switcher_input" id="model_status{{$m['id']}}" name="status" value="1" {{ $m['status'] == 1 ? 'checked':'' }} onclick="toogleStatusModal(event,'model_status{{$m['id']}}','model-status-on.png','model-status-off.png','{{translate('Want_to_Turn_ON')}} {{$m['defaultname']}} {{translate('status')}}','{{translate('Want_to_Turn_OFF')}} {{$m['defaultname']}} {{translate('status')}}',`<p>{{translate('if_enabled_this_model_will_be_available_on_the_website_and_customer_app')}}</p>`,`<p>{{translate('if_disabled_this_model_will_be_hidden_from_the_website_and_customer_app')}}</p>`)">
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center gap-2">
                                                    ...
                                                    href="{{route('admin.model.update',[$m['id']])}}">
                                                    <i class="tio-edit"></i>
                                                    </a>
...
@push('script')
    <script>
        $(document).on('click', '.delete', function () {
            var id = $(this).attr("id");
            Swal.fire({
                title: "{{ translate('are_you_sure_delete_this_model')}}?",
