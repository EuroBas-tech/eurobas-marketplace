                            ...
                            <div class="row">
                                <div class=" {{ theme_root_path() == 'theme_aster'?'col-lg-6':'col-lg-12 d-flex gap-3' }}">
                                    ...
                                    @foreach(json_decode($language) as $lang)
                                        <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                            ...
                                            <label class="title-color"
                                                   for="exampleFormControlInput1">{{translate('category_name')}}<span class="text-danger">*</span>
                                                ({{strtoupper($lang)}})</label>
                                            <input type="text" name="name[]" class="form-control"
                                                   placeholder="{{translate('Category')}}" {{$lang == $default_lang? 'required':''}}>
                                        </div>
                                    ...
                                    <div class="form-group w-100">
                                        <label class="title-color"
                                               for="exampleFormControlSelect1">{{translate('category_type')}}
                                            <span class="text-danger">*</span></label>
                                        <select id="exampleFormControlSelect1" name="category_type"
                                                class="form-control" required>
                                            <option value="" selected disabled>{{translate('select_category_type')}}</option>
                                            <option value="vehicles">{{translate('vehicles')}}</option>
                                            <option value="real estate">{{translate('real_estate')}}</option>
                                            <option value="electronics">{{translate('electronics')}}</option>
                                            <option value="home appliances">{{translate('home_appliances')}}</option>
                                            <option value="industrial machines">{{translate('industrial_machines')}}</option>
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                                            placeholder="{{translate('search_by_Category')}}" aria-label="Search orders" value="{{ $search }}" required>
...
@push('script')
    ...
    <script>
        $(document).on('click', '.delete', function () {
            var id = $(this).attr("id");
            Swal.fire({
                title: "{{translate('are_you_sure_to_delete_this_category')}}?",
