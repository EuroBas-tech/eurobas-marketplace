                        ...
                        <form action="{{route('admin.banner.update',[$banner['id']])}}" method="post" enctype="multipart/form-data"
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <div class="input_field_for_main_banner {{$banner['banner_type'] !='Main Banner'?'d-none':''}}">
                                        <hr>
                                        <h6 class="title-color text-capitalize mb-3">{{ translate('Text_Overlay_on_Banner') }}</h6>

                                        <div class="form-group mb-3">
                                            <label class="title-color d-flex align-items-center gap-2">
                                                <input type="checkbox" name="show_text" value="1"
                                                    id="show_text_check" {{ $banner['show_text'] ? 'checked' : '' }}>
                                                {{ translate('Show_text_on_banner') }}
                                            </label>
                                        </div>

                                        <div id="text-overlay-fields" style="{{ $banner['show_text'] ? '' : 'display:none;' }}">
                                            <div class="form-group mb-3">
                                                <label class="title-color text-capitalize">{{ translate('text_position') }}</label>
                                                <select name="text_position" class="form-control">
                                                    <option value="top" {{ ($banner['text_position'] ?? 'center') == 'top' ? 'selected' : '' }}>{{ translate('Top') }}</option>
                                                    <option value="center" {{ ($banner['text_position'] ?? 'center') == 'center' ? 'selected' : '' }}>{{ translate('Center') }}</option>
                                                    <option value="bottom" {{ ($banner['text_position'] ?? 'center') == 'bottom' ? 'selected' : '' }}>{{ translate('Bottom') }}</option>
                                                </select>
                                            </div>
                                            ...
                                            <div class="form-group mb-3">
                                                ...
                                                <select name="text_size" class="form-control">
                                                    <option value="small" {{ ($banner['text_size'] ?? 'large') == 'small' ? 'selected' : '' }}>{{ translate('Small') }}</option>
                                                    <option value="medium" {{ ($banner['text_size'] ?? 'large') == 'medium' ? 'selected' : '' }}>{{ translate('Medium') }}</option>
                                                    <option value="large" {{ ($banner['text_size'] ?? 'large') == 'large' ? 'selected' : '' }}>{{ translate('Large') }}</option>
                                                </select>
                                            </div>
                                            ...
                                            <div class="form-group mb-3">
                                                ...
                                                <select name="text_color" class="form-control">
                                                    <option value="white" {{ ($banner['text_color'] ?? 'white') == 'white' ? 'selected' : '' }}>{{ translate('White') }}</option>
                                                    <option value="orange" {{ ($banner['text_color'] ?? 'white') == 'orange' ? 'selected' : '' }}>{{ translate('Orange') }}</option>
                                                    <option value="blue" {{ ($banner['text_color'] ?? 'white') == 'blue' ? 'selected' : '' }}>{{ translate('Blue') }}</option>
                                                    <option value="gold" {{ ($banner['text_color'] ?? 'white') == 'gold' ? 'selected' : '' }}>{{ translate('Gold') }}</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="col-md-6 d-flex flex-column justify-content-center">
                                    <div>
                                        ...
                                        <p>{{ translate('banner_Image_ratio_is_not_same_for_all_sections_in_website') }}. {{ translate('please_review_the_ratio_before_upload') }}</p>
                                         ...
                                         @if(theme_root_path() == 'theme_aster')
                                         <div class="input_field_for_main_banner {{$banner['banner_type'] !='Main Banner'?'d-none':''}}"
                                              id="aster-title-fields"
                                              style="{{ $banner['show_text'] ? '' : 'display:none;' }}">
