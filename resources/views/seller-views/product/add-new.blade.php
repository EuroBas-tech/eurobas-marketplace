                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 col-lg-4 col-xl-3">
                            <div class="form-group">
                                <label for="name" class="title-color">{{ translate('category') }}
                        ...
                        <div class="col-md-6 col-lg-4 col-xl-3 ">
                            <div class="form-group">
                                <label class="title-color">{{ translate('The origin of the product facility') }}<span class="text-danger" > *</span></label>
                                <select class="js-example-basic-multiple form-control" name="origin">
                                    <option  value="{{ old('origin') }}"  disabled selected>{{ translate('Select Country') }}</option>
...
<div class="card mt-3 rest-part physical_product_show">
    <div class="card-header">
        <div class="d-flex gap-2">
            <i class="tio-user-big"></i>
            <h4 class="mb-0">{{ translate('Select Country Shipping') }}<span class="text-danger" > *</span></h4>
        </div>
    </div>
    <div class="card-body">
        <div class="row align-items-end">
            <div class="col-md-6">
                <label for="shipping-selector">{{ translate('Select Shipping Country') }}</label>
                <select
                    class="js-example-basic-multiple js-states js-example-responsive form-control color-var-select"
                    name="countries[]" multiple="multiple" id="shipping-selector">
                        @foreach (COUNTRIES as $country)
            ...
            <div class=" d-flex align-items-center gap-2 ">
                <button type="button" id="select-all" class="btn btn-primary"> {{ translate('select_all') }} </button>
                <button type="button" id="clear-all" class="btn btn-secondary">{{ translate('clear_all') }}</button>
            </div>
        </div>
    </div>
</div>
                ...
                <div class="card-body">
                    <div class="row align-items-end">
                        <div class="col-md-6 col-lg-4 col-xl-3">
                            <div class="form-group">
                                <div class="d-flex gap-2 mb-2">
                                    <label class="title-color mb-0">{{ translate('price') }} ({{ \App\CPU\BackEndHelper::currency_symbol() }})</label>
                                ...
                                <input type="number" min="0" step="0.01"
                                        placeholder="{{ translate('price') }}" name="unit_price"
                        ...
                        <div class="col-md-6 physical_product_show" id="shipping_cost_multy">
                            <div class="form-group">
                                <div class="form-control h-auto min-form-control-height d-flex align-items-center flex-wrap justify-content-between gap-0">
                                    <div class="d-flex gap-2">
                ...
                <div class="card-body">
                    <div class="row align-items-end">
                        <div class="col-md-6">
                            ...
                            <div id="color-selector-container" class="d-none mb-3 align-items-center w-75 gap-1 flex-wrap" style="
                                    background: #808080cc;
                                    padding: 10px;
                                    border-radius: 8px;" 
                                >
                                @foreach (\App\Model\Color::orderBy('name', 'asc')->get() as $key => $color)
                                    <span class="rounded color-card-selector" style="width:18px;height:18px;background:{{ $color->code }}" name="{{ $color->code }}"></span>
                                @endforeach
                            </div>
...
@push('script')
        ...
        $(function() {
            $('#color_switcher').click(function(){
                ...
                if ($('#color_switcher').prop('checked')) {
                    ...
                    $('#color-selector-container').removeClass('d-none').addClass('d-flex');
                } else {
                    ...
                    $('#color-selector-container').addClass('d-none').removeClass('d-flex');
                }
            });
    ...
    <script>
        ...
        $('input[name="colors_active"]').on('change', function() {
         ...
         $('input[name="shipping_active"]').on('change', function() {

        if (!$('input[name="shipping_active"]').is(':checked')) {
            $('#shipping-selector').prop('disabled', true);
        } else {
            $('#shipping-selector').prop('disabled', false);
        }
        });
        ...
        $('.color-card-selector').on('click', function() {
            let color = $(this).attr('name');
            let selectedColors = $('#colors-selector').val() || []; 

            if (!selectedColors.includes(color)) {
                selectedColors.push(color);
            }

            $('#colors-selector').val(selectedColors).trigger('change');
        });
    ...
    $('#select-all').click(function() {
        $('#shipping-selector > option').prop('selected', true);
        $('#shipping-selector').trigger('change'); // لتحديث الـ Select2 إذا كنت تستخدمه
    });
    
    
    document.getElementById('clear-all').addEventListener('click', function() {
    // إلغاء تحديد جميع الخيارات في القائمة المنسدلة
