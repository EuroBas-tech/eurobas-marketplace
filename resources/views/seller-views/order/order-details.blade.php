                        ...
                        <div class="d-flex flex-wrap gap-10 justify-content-between mb-4">
                            ...
                            <div class="text-sm-right">
                                <div class="d-flex flex-wrap gap-10 justify-content-end">
                                    ...
                                    <a class="btn btn--primary px-4" target="_blank"
                                    href="{{route('seller.orders.generate-shipping',[$order['id']])}}">
                                        <i class="tio-print mr-1"></i> {{translate('print__shipping')}}
                                    </a>
                                </div>
                                <div class="d-flex flex-column gap-2 mt-3">
                                    ...
                                    <div class="order-status d-flex justify-content-sm-end gap-10 text-capitalize">
                                        ...
                                        @elseif($order['order_status']=='delivered' || $order['order_status']=='confirmed' || $order['order_status']=='delivery_confirmed')
                ...
                <div class="card">
                    <div class="card-body text-capitalize d-flex flex-column gap-4">
                        ...
                        <select name="order_status" onchange="order_status(this.value)"
                            ...
                            <option {{ $order->order_status != 'pending' ? 'disabled' : '' }}
                                value="pending" {{$order->order_status == 'pending'?'selected':''}} > {{translate('pending')}}</option>

                            <option {{ $order->order_status != 'pending' && $order->order_status != 'confirmed' ? 'disabled' : '' }}
                                value="confirmed" {{$order->order_status == 'confirmed'?'selected':''}} > {{translate('confirmed')}}</option>

                            <option {{ $order->order_status != 'pending' && $order->order_status != 'confirmed' && $order->order_status != 'processing' ? 'disabled' : '' }}
                                value="processing" {{$order->order_status == 'processing'?'selected':''}} >{{translate('packaging')}}</option>

                            <option {{ $order->order_status != 'pending' && $order->order_status != 'confirmed' && $order->order_status != 'processing' && $order->order_status != 'shipped' ? 'disabled' : '' }}
                                value="shipped" {{$order->order_status == 'shipped'?'selected':''}} >{{translate('shipped')}}</option>

                            <option disabled
                                value="delivered" {{$order->order_status == 'delivered'?'selected':''}} >{{translate('delivered')}}</option>

                            <option disabled
                                value="delivery_confirmed" {{$order->order_status == 'delivery_confirmed'?'selected':''}} >{{translate('delivery_confirmed')}}</option>

                            <option {{ $order->order_status == 'canceled' ? 'disabled' : '' }}
                                value="returned" {{$order->order_status == 'returned'?'selected':''}} > {{translate('returned')}}</option>
                            <option {{ $order->order_status == 'canceled' || $order->order_status == 'delivered' ? 'disabled' : '' }}
                                value="failed" {{$order->order_status == 'failed'?'selected':''}} >{{translate('failed_to_Deliver')}}</option>
                            <option {{ $order->order_status == 'delivered' ? 'disabled' : '' }}
                        ...
                        <div class="d-flex justify-content-between align-items-center gap-10 form-control h-auto flex-wrap">
                            ...
                            <div class="d-flex justify-content-end min-w-100 align-items-center gap-2">
                                ...
                                <label class="switcher payment-status-text">
                                    <input class="switcher_input payment_status" {{ $order['order_status'] == 'delivery_confirmed' ? 'disabled' : '' }} type="checkbox" name="status" value="{{$order->payment_status}}"
                                ...
                                @if($order['order_status'] != 'canceled')
                                    <div>
                                        ...
                                        class="btn btn-primary" href="#">
                                            ...
                                            @if($order['order_status'] == 'delivery_confirmed' || $order['order_status'] == 'delivered')
                                                <span>
                                                    {{ translate('show_shipping_info') }}
                                                </span>
                                            @else
                                                <span>
                                                    {{ $order->delivery_service_name != null && $order->third_party_delivery_tracking_id != null ? translate('update_shipping_info') : translate('add_shipping_info') }}
                                                </span>
                                            @endif
                                        </a>
                                    </div>
                                ...
                                <div class="p-2 bg-light rounded">
                                    <div class="media m-1 gap-3">
                                        ...
                                        <div class="media-body">
                                            ...
                                            <span class="fz-12 title-color">{{translate('track_ID')}} :  {{$order->third_party_tracking_number}}</span>
                                        </div>
                                    </div>
                                </div>
                            </li>
                            ...
                            <form action="{{route('seller.orders.update-deliver-info')}}" method="POST">
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <input {{ $order['order_status'] == 'delivery_confirmed' || $order['order_status'] == 'delivered' ? 'disabled' : '' }} class="form-control" type="text" name="delivery_service_name" value="{{$order['delivery_service_name']}}" id="" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="">{{translate('tracking_number')}}</label>
                                        <input {{ $order['order_status'] == 'delivery_confirmed' || $order['order_status'] == 'delivered' ? 'disabled' : '' }} class="form-control" type="text" name="third_party_tracking_number" value="{{$order['third_party_tracking_number']}}" id="">
                                    </div>
                                    <button {{ $order['order_status'] == 'delivery_confirmed' || $order['order_status'] == 'delivered'  ? 'disabled' : '' }} class="btn btn--primary" type="submit">{{translate('update')}}</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script>
        ...
        function show_location_map(){
            ...
            google.maps.event.addListener(marker, 'click', (function (marker) {
                return function () {
                    infowindow.setContent("<div class='float-left'><img class='__inline-5' src='{{asset('public/storage/app/public/profile/')}}{{$order->customer->image??""}}'></div><div class='float-right __p-10'><b>{{$order->customer->f_name??""}} {{$order->customer->l_name??""}}</b><br/>{{$shipping_address->address??""}}</div>");
