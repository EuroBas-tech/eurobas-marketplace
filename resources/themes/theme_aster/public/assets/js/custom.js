...
// //popup for desktop
// document.querySelector('.openPopupBtn').addEventListener('mouseover', function() {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    var elements = document.querySelectorAll('.body-font');
    if (direction === 'rtl') {
        ...
        elements.forEach(function(element) {
            element.style.fontFamily = "'Mulish', sans-serif";
        });
    }
});
...
// //popup for mobile
// document.querySelector('.openPopupBtnmob').addEventListener('mouseover', function() {
//     document.getElementById('popupmob').style.display = 'flex';
// });
...
//     var currencies = document.querySelectorAll('.custom-select-currency-list');
//     currencies.forEach(function(currency) {
//         var text = currency.textContent.toLowerCase();
//         var match = text.includes(searchQuery);
//         currency.style.display = match ? 'block' : 'none';
//     });
// });
// document.getElementById('currencySearchInput').addEventListener('focus', function() {
//     document.querySelector('.custom-select-currency').style.display = 'block';
// });
...
//         var text = country.textContent.toLowerCase().replace(/\s+/g, ' ').trim();
//         var match = text.includes(searchQuery);
...
//         var selectElement = document.querySelector('.custom-select').style.display = 'block';
...
$(".search-bar-input-mobile").on("input", function () {
    if ($(this).val() === "") {
      $(".search-result-box").hide();
    }
  });

$(".search-bar-input-mobile").on("search", function () {
    $(".search-result-box").hide();
});


/** start website main search  **/

// Voice Search Implementation with Multi-language Support
let recognitionInstances = new Map(); // Store recognition instances for each search box
let isListeningMap = new Map(); // Track listening state for each search box
let silenceTimerMap = new Map(); // Store silence timers for each search box
let accumulatedTextMap = new Map(); // Store accumulated text for each search box
...
const languageMapping = {
    ...
    'ru': 'ru-RU',    // Русский
    'fr': 'fr-FR',    // Français
    'ae': 'ar-AE',    // العربية (Arabic) - Changed from ar-SA to ar-AE for better support
    'ar': 'ar-AE',    // Alternative Arabic mapping
    'gr': 'el-GR',    // ελληνικά (Greek)
    'pl': 'pl-PL',    // Polski
    'ro': 'ro-RO',    // Română
    'ua': 'uk-UA',    // Украiнська (Ukrainian)
    'bg': 'bg-BG',    // Bulgarski
    'pt': 'pt-PT',    // Português
    'rs': 'sr-RS',    // Српски (Serbian)
    'dk': 'da-DK',    // Dansk
    'se': 'sv-SE',    // Svenska
    'fi': 'fi-FI',    // Suomi
    'no': 'no-NO',    // Norsk
...
function getSpeechRecognitionLanguage() {
    ...
    const speechLang = languageMapping[currentLang] || 'en-US';
    console.log(`Current session language: ${currentLang}, Speech recognition language: ${speechLang}`);
    return speechLang;
}

// Check if speech recognition is supported for current language
...
// Get unique identifier for search box
function getSearchBoxId($container) {
    if ($container.hasClass('desktop-search-container')) {
        return 'desktop';
    } else if ($container.hasClass('mobile-search-container')) {
        return 'mobile';
    } else {
        // Fallback - use container's index
        return 'search-' + $('.search-box').index($container.find('.search-box'));
    }
}

// Initialize Speech Recognition for a specific search box
function initializeSpeechRecognition($container) {
    if (!isSpeechRecognitionSupported()) {
        console.log('Speech recognition not supported in this browser');
        return false;
    }
    
    const searchBoxId = getSearchBoxId($container);
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    const recognition = new SpeechRecognition();
    
    // Configure recognition settings with current language
    ...
    console.log(`Speech recognition initialized for ${searchBoxId} with language:`, recognition.lang);
    ...
    recognition.onresult = function(event) {
        ...
        for (let i = event.resultIndex; i < event.results.length; i++) {
            ...
            // If no confidence available, use first result
            const transcript = bestTranscript || result[0].transcript;
            
            if (result.isFinal) {
                finalTranscript += transcript;
                console.log(`Final transcript for ${searchBoxId}: "${transcript}" (confidence: ${result[0].confidence || 'N/A'})`);
            } else {
                interimTranscript += transcript;
            }
        }
        
        // Update search input with current transcript for this specific search box
        let currentText = '';
        let accumulatedText = accumulatedTextMap.get(searchBoxId) || '';
        
        if (finalTranscript.trim()) {
            accumulatedText += finalTranscript;
            accumulatedTextMap.set(searchBoxId, accumulatedText);
            currentText = accumulatedText.trim();
        } else if (interimTranscript.trim()) {
            currentText = (accumulatedText + interimTranscript).trim();
        }
        
        if (currentText) {
            $container.find('.search-bar-input-mobile').val(currentText);
            
            // Activate sound waves when speaking
            $container.find('.sound-waves').removeClass('silent').addClass('speaking');
            
            // Clear existing silence timer
            clearTimeout(silenceTimerMap.get(searchBoxId));
            
            // Set new silence timer (2-3 seconds of silence)
            const silenceTimer = setTimeout(function() {
                // Switch to silent waves
                $container.find('.sound-waves').removeClass('speaking').addClass('silent');
                
                // After additional delay, stop recording and search
                setTimeout(function() {
                    if (isListeningMap.get(searchBoxId) && currentText.trim()) {
                        console.log(`Stopping voice search for ${searchBoxId} with final text: "${currentText}"`);
                        stopVoiceSearch($container);
                        // Trigger the search functionality for this specific search box
                        $container.find('.search-bar-input-mobile').trigger('keyup');
                    }
                }, 1500); // Additional 1.5 seconds in silent state
                
            }, 2500); // Increased to 2.5 seconds of silence for better multi-language support
            
            silenceTimerMap.set(searchBoxId, silenceTimer);
        }
    };
    
    // Handle recognition start
    recognition.onstart = function() {
        console.log(`Speech recognition started for ${searchBoxId} with language:`, recognition.lang);
        accumulatedTextMap.set(searchBoxId, ''); // Reset accumulated text for new session
    };
    
    // Handle recognition errors
    recognition.onerror = function(event) {
        console.error(`Speech recognition error for ${searchBoxId}:`, event.error);
        
        // Get current language for user-friendly error messages
        const currentLang = getCurrentLanguage();
        ...
        if (event.error === 'not-allowed') {
            ...
            console.log(`No speech detected for ${searchBoxId}, continuing to listen...`);
            // Don't stop for no-speech in continuous mode
        } else if (event.error === 'language-not-supported') {
            console.warn(`Language ${recognition.lang} not supported for ${searchBoxId}, falling back to English`);
            recognition.lang = 'en-US';
            // Don't stop, let it continue with English
        } else if (event.error === 'network') {
            alert('Network error occurred. Please check your internet connection.');
            stopVoiceSearch($container);
        } else if (event.error === 'aborted') {
            console.log(`Speech recognition was aborted for ${searchBoxId}`);
            stopVoiceSearch($container);
        } else {
            console.warn(`Speech recognition error for ${searchBoxId}: ${event.error}`);
    ...
    recognition.onend = function() {
        console.log(`Speech recognition ended for ${searchBoxId}`);
        if (isListeningMap.get(searchBoxId)) {
            // If we're still supposed to be listening, restart
            try {
                // Update language in case it changed
                const newLang = getSpeechRecognitionLanguage();
                if (recognition.lang !== newLang) {
                    recognition.lang = newLang;
                    console.log(`Updated language for ${searchBoxId} to:`, newLang);
                }
                recognition.start();
            } catch (e) {
                console.error(`Error restarting recognition for ${searchBoxId}:`, e);
                stopVoiceSearch($container);
            }
        }
    };
    ...
    recognitionInstances.set(searchBoxId, recognition);
    
    return true;
}

// Start voice search function for a specific search box
function startVoiceSearch($container) {
    if (!isSpeechRecognitionSupported()) {
    ...
    const searchBoxId = getSearchBoxId($container);
    let recognition = recognitionInstances.get(searchBoxId);
    
    if (!recognition) {
        // Initialize if not already done
        const initialized = initializeSpeechRecognition($container);
        if (!initialized) {
            alert('Could not initialize voice search. Please try again.');
            return;
        }
        recognition = recognitionInstances.get(searchBoxId);
    }
    
    isListeningMap.set(searchBoxId, true);
    accumulatedTextMap.set(searchBoxId, ''); // Reset for new session
    
    // Update recognition language before starting
    const speechLang = getSpeechRecognitionLanguage();
    recognition.lang = speechLang;
    console.log(`Starting voice search for ${searchBoxId} with language:`, recognition.lang);
    
    // Change button appearance to indicate recording
    const $voiceBtn = $container.find('.search_voice');
    $voiceBtn.addClass('recording');
    $voiceBtn.find('.voice-search-icon')
        .removeClass('bi-mic')
        .addClass('bi-mic-fill');
    
    // Add sound waves visualization
    const soundWaves = $(`
        <div class="sound-waves silent">
    ...
    `);
    $voiceBtn.append(soundWaves);
    
    // Clear the search input for this specific search box
    $container.find('.search-bar-input-mobile').val('').focus();
    
    try {
        recognition.start();
    } catch (e) {
        console.error(`Error starting recognition for ${searchBoxId}:`, e);
        alert('Could not start voice search. Please try again.');
        stopVoiceSearch($container);
    }
}

// Stop voice search function for a specific search box
function stopVoiceSearch($container) {
    const searchBoxId = getSearchBoxId($container);
    isListeningMap.set(searchBoxId, false);
    
    // Clear timers
    clearTimeout(silenceTimerMap.get(searchBoxId));
    silenceTimerMap.delete(searchBoxId);
    
    // Reset button appearance
    const $voiceBtn = $container.find('.search_voice');
    $voiceBtn.removeClass('recording');
    $voiceBtn.find('.voice-search-icon')
        .removeClass('bi-mic-fill')
        .addClass('bi-mic')
        .css('color', '');
    
    // Remove sound waves
    $container.find('.sound-waves').remove();
    
    // Stop recognition
    const recognition = recognitionInstances.get(searchBoxId);
    if (recognition) {
        try {
            recognition.stop();
        } catch (e) {
            console.error(`Error stopping recognition for ${searchBoxId}:`, e);
        }
    }
}

// Voice search button click handler and initialization
$(document).ready(function() {
    // Test speech recognition support on page load
    if (isSpeechRecognitionSupported()) {
        console.log('Speech recognition is supported');
    } else {
        console.warn('Speech recognition is not supported in this browser');
        // Optionally hide the voice search buttons
        // $('.search_voice').hide();
    }
    
    // Handle voice search button clicks for all search boxes
    $(document).on('click', '.search_voice', function(e) {
        e.preventDefault();
        
        const $container = $(this).closest('.desktop-search-container, .mobile-search-container');
        const searchBoxId = getSearchBoxId($container);
        
        if (!isListeningMap.get(searchBoxId)) {
            startVoiceSearch($container);
        } else {
            stopVoiceSearch($container);
        }
    });
    
    // Handle clicking outside to stop voice search
    $(document).click(function(e) {
        const $target = $(e.target);
        
        // Check each active search box
        isListeningMap.forEach((isListening, searchBoxId) => {
            if (isListening) {
                const $container = searchBoxId === 'desktop' ? $('.desktop-search-container') : $('.mobile-search-container');
                
                if (!$target.closest('.search_voice, .search-bar', $container).length) {
                    stopVoiceSearch($container);
                }
            }
        });
    ...
    // Handle Escape key to stop voice search
    ...
    $(document).on('languageChanged', function() {
        ...
        recognitionInstances.forEach((recognition, searchBoxId) => {
            recognition.lang = newLang;
            console.log(`Language changed, updated speech recognition for ${searchBoxId} to:`, newLang);
        });
    });
});

// Your existing search functionality (updated to work with multiple search boxes)
let typingTimerMap = new Map();
const typingDelay = 1000;

$(document).on('keyup', '.search-bar-input-mobile', function() {
    const $input = $(this);
    const $container = $input.closest('.desktop-search-container, .mobile-search-container');
    const searchBoxId = getSearchBoxId($container);
    
    // Clear existing timer for this search box
    clearTimeout(typingTimerMap.get(searchBoxId));
    
    const timer = setTimeout(function() {
        $container.find(".search-card").css("display", "block");
        ...
        if (title.length > 0) {
            $.get({
                ...
                success: function (data) {
                    $container.find(".search-result-box").empty().html(data.result);
                },
                complete: function () {
                    $container.find("#loading").removeClass("d-grid");
                },
            });
        } else {
            $container.find(".search-result-box").empty();
        }
    }, typingDelay);
...
function addWishlist(ad_id, action_url) {
    $.ajaxSetup({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content"),
        },
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.value == 1) {
                ...
                $(`.wishlist-${ad_id}`).addClass("wishlist_icon_active");
                $(".wishlist_count_status").html(
                    parseInt($(".wishlist_count_status").html()) + 1
                );
                $("#ad-wishlist-count").html(
                    parseInt($("#ad-wishlist-count").html()) + 1
                );
            } else if (data.value == 2) {
                $(`.wishlist-${ad_id}`).removeClass(
...
$("#contact_with_seller_form").on("submit", function (e) {
    ...
    $.ajax({
        ...
        success: function (respons) {
            ...
            if(respons.error_message) {
                toastr.error(respons.error_message);
            } else {
                toastr.success(
...
function submitMakeAnOfferForm() {
    let messages_form = $("#make_an_offer_form");
    $.ajaxSetup({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content"),
        },
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if(response.error_message) {
                ...
                toastr.success(
                    $("#make_an_offer_form").data("success-message"),
                    {
                        CloseButton: true,
                        ProgressBar: true,
                    }
            ...
            $("#make_an_offer_form").trigger("reset");
            $("#makeAnOfferModal").modal("hide");
        },
        error: function (xhr) {
            toastr.error(xhr.responseText);
            $('#make_an_offer_form').modal('hide');
        },

    });
}

function submitAskingPriceForm() {
    let messages_form = $("#asking_price_form");
    $.ajaxSetup({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content"),
        },
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if(response.error_message) {
                ...
                toastr.success(
                    $("#asking_price_form").data("success-message"),
                    {
                        CloseButton: true,
                        ProgressBar: true,
                    }
            ...
            $("#asking_price_form").trigger("reset");
            $("#askingPriceModal").modal("hide");
        },
        error: function (xhr) {
            toastr.error(xhr.responseText);
            $('#asking_price_form').modal('hide');
        },
    });
}
...
function removeWishlist(ad_id, url) {
    $.ajaxSetup({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content"),
        },
...
function formUrlChange(t){
    ...
    $(t).closest('form').find('[name="model_id"]').prop('disabled', false);
