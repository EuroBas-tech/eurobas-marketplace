                ...
                <div class="col-lg-10">
                    <div class="card h-lg-100 card-custom-shadow">
                        <div class="card-body p-4 pb-4">
                            <h1 class="pb-3 fs-30" >{{translate('edit_add')}}</h1>
                            ...
                            <div class="mt-4">
                                <form  action="" method="POST" id="ads-store-form" enctype="multipart/form-data">
                                    ...
                                    <div class="row gy-4">
                                        ...
                                        @include('theme-views.ad.edit-pages.partials.technical-information')
                                        ...
                                        && $ad->category->slug !== 'bicycles')
                                            @include('theme-views.ad.edit-pages.partials.environmental-information')
                                        @endif
                                        ...
                                        && $ad->category->slug !== 'vehicle-accessories' && $ad->category->slug !== 'spare-parts')
                                            @include('theme-views.ad.edit-pages.partials.vehicle-options')
                                        @endif

                                        @if($ad->category->slug !== 'bicycles' && $ad->category->slug !== 'motorcycles'
                                        && $ad->category->slug !== 'vehicle-accessories' && $ad->category->slug !== 'spare-parts')
                                            @include('theme-views.ad.edit-pages.partials.additional-information')
                                        @endif
...
function addMoreImage(input, targetSection) {
    ...
    if (invalidFiles.length > 0) {
        toastr.error('Only JPG, JPEG, PNG, WEBP, AVIF files are acceptable');
        input.value = '';
        return;
    }

    if (oversizedFiles.length > 0) {
        toastr.error('Maximum file size is 4MB');
        input.value = '';
        return;
    }
    ...
    const remainingSlots = 10 - imageBoxes.length;

    if (files.length > remainingSlots) {
        toastr.error(`{{translate('maximum_10_images_allowed')}} (You can only add ${remainingSlots} more)`);
...
// ===== EDIT PAGE SPECIFIC CODE - Load existing images on page load =====

// Load existing thumbnail image on page load
...
$(window).on("load", function () {
    $('.ad-images').each(function(index, image) {
        ...
        let uploadFile = $image.closest('.upload-file');

        // Add position-relative class if not present
        if (!uploadFile.hasClass('position-relative')) {
            uploadFile.addClass('position-relative');
        }
    ...
    // Make sure the last upload box (empty one) has the correct onchange handler
    const lastUploadInput = $('#additional_Image_Section .upload-file:last-child .upload-file__input');
    if (lastUploadInput.length && !lastUploadInput.attr('onchange')) {
        lastUploadInput.attr('onchange', "addMoreImage(this, '#additional_Image_Section')");
    }
});
