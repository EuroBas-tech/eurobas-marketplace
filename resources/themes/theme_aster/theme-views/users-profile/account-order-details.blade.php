                ...
                <div class="col">
                    <div class="d-lg-none row align-items-center mt-2 mb-3">
                        ...
                        <div class="col-3">
                            <div class="d-flex gap-3 justify-content-end mt-2">
                                @if(($order->order_status=='delivered' || $order->order_status=='delivery_confirmed') && $order->order_type == 'default_type')
                    ...
                    <div class="card h-100">
                        <div class="card-body p-lg-4">
                            ...
                            <div class="mt-4 card">
                                <div class="card-body">
                                    <div class="table-responsive rounded dashed-border">
                                        ...
                                        @foreach ($order->details as $key=>$detail)
                                            ...
                                            @endif
                                        @endforeach
                                        <table class="table align-middle rounded m-0">
                                            <thead class="bg-cool-blue rounded">
                                            <tr>
                                                ...
                                                <th class="border-0 text-end" {{ ($order->order_type == 'default_type' && ($order->order_status=='delivered') || $order->order_status=='delivery_confirmed') ? 'colspan="2"':'' }}>{{translate('Total')}}</th>
                                                @if($order->order_type == 'default_type' && (($order->order_status=='delivered' || $order->order_status=='delivery_confirmed') || ($order->payment_status == 'paid' && $digital_product)))
                                                    <th class="border-0 text-center">{{translate('Action')}}</th>
                                                @elseif($order->order_type != 'default_type' && ($order->order_status=='delivered' || $order->order_status=='delivery_confirmed'))
                                            ...
                                            @foreach ($order->details as $key=>$detail)
                                                ...
                                                @if($product)
                                                    <tr>
                                                        <td>
                                                            <div class="media gap-3 align-items-center">
                                                                ...
                                                                <div class="media-body d-flex gap-1 flex-column">
                                                                    ...
                                                                    @if($detail->variant)
                                                                        @if(in_array($detail->variant, $colors))
                                                                            <small class="d-flex align-items-center" ><span>{{translate('variant')}}</span>
                                                                                : <span class="show-color-in-card mx-1" style="background:{{$detail->variant}}" ></span></small>
                                                                        @else
                                                        ...
                                                        @php($order_details_date = \App\CPU\order_status_history($order->id, 'delivered') ?: null)
                                                        @php($refund_limit = \App\Model\BusinessSetting::where('type','refund_day_limit')->first()->value)
                                                        @php($length = $order_details_date != null ? $order_details_date->diffInDays($current_date) : null)
                                                        <td>
                                                            ...
                                                            <div class="d-flex justify-content-center gap-2">
                                                                @if($order->order_type == 'default_type')
                                                                    @if($order->order_status=='delivered' || $order->order_status=='delivery_confirmed')
                                                                        ...
                                                                        @if($length < $refund_limit && $detail->refund_request == 0)
                                                                            <button
                                        ...
                                        <div class="col-xl-6 col-lg-7 col-md-8 col-sm-10">
                                            <div class="d-flex flex-column gap-3 text-dark">
                                                <div
                                                    ...
                                                    <div class="fw-bold" >{{translate('Item')}}</div>
                                                    <div class="fw-bold" >{{$order->details->count()}}</div>
                                                </div>
                                                <div
                                                    class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                                                    <div class="fw-bold" >{{translate('Subtotal')}}</div>
                                                    <div class="fw-bold currency-font" >{{\App\CPU\Helpers::currency_converter($summary['subtotal'])}}</div>
                                                </div>
                                                <div
                                                    class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                                                    <div class="fw-bold" >{{translate('tax_fee')}}</div>
                                                    <div class="fw-bold currency-font" >{{\App\CPU\Helpers::currency_converter($summary['total_tax'])}}</div>
                                                </div>
                                                @if($order->order_type == 'default_type')
                                                    <div
                                                        class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                                                        <div class="fw-bold" >{{translate('Shipping_Fee')}}</div>
                                                        <div class="fw-bold currency-font" >{{\App\CPU\Helpers::currency_converter($summary['total_shipping_cost'] - ($order['is_shipping_free'] ? $order['extra_discount'] : 0))}}</div>
                                                    </div>
                                                @endif
                                                <div
                                                    class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                                                    <div class="fw-bold" >{{translate('Discount_on_product')}}</div>
                                                    <div class="fw-bold currency-font" > {{\App\CPU\Helpers::currency_converter($summary['total_discount_on_product'])}}</div>
                                                </div>
                                                <div
                                                    class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                                                    <div class="fw-bold" >{{translate('Coupon_Discount')}}</div>
                                                    <div class="fw-bold currency-font" >
                                                ...
                                                @if($order->order_type != 'default_type')
                                                    <div
                                                        ...
                                                        <div class="fw-bold" >{{translate('extra_Discount')}}</div>
                                                        <div class="fw-bold currency-font" >
                                                ...
                                                <div
                                                    class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                                                    <h4 class="fw-bold" >{{translate('Total')}}</h4>
                                                    <h2 class="text-primary fw-bold currency-font">{{\App\CPU\Helpers::currency_converter($order->order_amount)}}</h2>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
