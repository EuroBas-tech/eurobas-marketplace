    ...
    <div class="container">
        @foreach($categories->where('position', 1) as $category)
            @if($category->ads()->when(session('show_by_country'), fn($q, $country) => $q->country($country['name']))->count() > 4)
                <div class="mb-4 wow fadeInUp" data-wow-delay=".2s">
                    <div class="d-flex justify-content-between align-items-center mb-3 wow fadeInLeft" data-wow-delay=".3s">
                    ...
                    <div>
                        <div data-category="{{$category->id}}" id="{{ preg_replace('/[^a-zA-Z0-9 ]/', '', str_replace(' ', '-', $category->name)) }}">
                            <div class="auto-col mobile-items-2 gap-2 gap-sm-3 recommended-product-grid wow fadeInUp" data-wow-delay=".4s" style="--minWidth: 12rem;">
