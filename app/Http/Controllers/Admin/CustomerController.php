...
class CustomerController extends Controller
{
    ...
    public function view(Request $request, $id)
    {
        ...
        $customer = User::with('paid_banners')->withCount('ads', 'paid_banners')->find($id);

        $is_profile_incompleted = Helpers::json_prevent_if_profile_incomplete();

        $user_native_language = null;
        $languages = BusinessSetting::where('type', 'language')->first();

        if($customer->native_language) {
            foreach (json_decode($languages['value'], true) as $key => $data) {
                if ($data['code'] == $customer->native_language) {
                    $user_native_language = $data['name'];
                    break;
                }
            }
        }
        ...
        if (isset($customer)) { 
            $query_param = []; 
            $ads = Ad::with('sponsor')->where('user_id', $id)->latest();
            $search = $request['search']; 
            $orders = Order::where(['customer_id' => $id, 'is_guest'=>'0']); 
            if ($request->has('search')) { 
                $ads = $ads->where(function ($q) use ($search) { 
            ...
            // paginate at the end
            $ads = $ads->paginate(Helpers::pagination_limit())->appends($query_param);

            return view('admin-views.customer.customer-view', compact('customer', 'ads', 'search', 'is_profile_incompleted', 'user_native_language')); 
        }
