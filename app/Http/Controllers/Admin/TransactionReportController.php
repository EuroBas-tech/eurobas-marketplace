...
class TransactionReportController extends Controller
{
    ...
    public function seller_report(Request $request)
    {
        ...
        $transactions = self::order_transaction_table_data_filter($request);
        $seller_wallet_actions = self::seller_wallet_table_data_filter($request);
        $seller_wallet_actions = $seller_wallet_actions->get();
        ...
        if(isset($seller_id) && $seller_id != 'all' && is_numeric($seller_id)) {
            
            $seller = Seller::with('wallet')->select('id', 'f_name', 'l_name')->find($seller_id);
            
            $seller_f_name = $seller->f_name ?? '';
            $seller_l_name = $seller->l_name ?? '';

            $data['seller_fullname'] = $seller_f_name.' '.$seller_l_name;
            $data['total_earning'] = $seller->wallet->total_earning ?? 0;
            $data['withdrawn'] = $seller_wallet_actions->sum('withdrawn');
            $data['commission_given'] = $seller_wallet_actions->sum('commission_given') ?? 0;
            $data['pending_withdraw'] = $seller_wallet_actions->sum('pending_withdraw') ?? 0;
            $data['delivery_charge_earned'] = $seller_wallet_actions->sum('delivery_charge_earned') ?? 0;
            $data['collected_cash'] = $seller_wallet_actions->sum('collected_cash') ?? 0;
            $data['total_tax_collected'] = $seller_wallet_actions->sum('total_tax_collected') ?? 0;
        }
        ...
        return view('admin-views.transaction.seller-reports', compact('customers', 'sellers', 'transactions', 'search', 'status', 'data',
            'from', 'to', 'customer_id', 'seller_id', 'payment_status', 'order_data', 'date_type', 'payment_data', 'order_transaction_chart'));
    }
            ...
            if($tran['order']) {
                ...
                if($tran['seller_is'] == 'seller'){
                    if($tran->order->shipping_responsibility == 'inhouse_shipping'){
                        ...
                        $admin_net_income += ($tran->order->coupon_discount_bearer == 'seller' && $tran->order->coupon->coupon_type == 'free_delivery') ? $seller_coupon_discount:0;
                ...
                $tranData[] = array(
                    ...
                    'Discounted Amount' => BackEndHelper::set_symbol(BackEndHelper::usd_to_currency($tran->order_details[0]->order_details_sum_price - $tran->order_details[0]->order_details_sum_discount - (isset($tran->order->coupon) && $tran->order->coupon->coupon_type == 'free_delivery'?0:$tran->order->discount_amount))),
    ...
    public function order_transaction_summary_pdf(Request $request)
    {
        ...
        foreach ($transactions as $transaction) {
            if($transaction->order) {
                ...
                $total_discounted_amount += $transaction->order_details[0]->order_details_sum_price - $transaction->order_details[0]->order_details_sum_discount - (isset($transaction->order->coupon) && $transaction->order->coupon->coupon_type != 'free_delivery'?$transaction->order->discount_amount:0);
    ...
    public function seller_wallet_table_data_filter($request)
    {
        $search = $request['search'];
        $from = $request['from'];
        $to = $request['to'];
        ...
        $seller_wallet_actions_query = SellerWalletAction::
            when($seller_id != 'all', function ($query) use ($seller_id) {
                $query->where('seller_id', $seller_id);
            });
        $transactions = self::date_wise_common_filter($seller_wallet_actions_query , $date_type, $from, $to);
