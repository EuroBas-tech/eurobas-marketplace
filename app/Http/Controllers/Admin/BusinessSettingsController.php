...
class BusinessSettingsController extends Controller
{
    ...
    public function productSettings()
    {
        ...
        return view('admin-views.business-settings.product-settings', compact('company_name','company_email','company_phone',
        'digital_product','brand','ad_images_size', 'maximum_ad_images_number'));
    }
    ...
    public function updateProductSettings(Request $request){
        ...
        BusinessSetting::updateOrInsert(['type' => 'ad_images_size'], [
            'value' => $request['ad_images_size']
        ]);
        
        BusinessSetting::updateOrInsert(['type' => 'maximum_ad_images_number'], [
    ...
    public function features_section_submit(Request $request)
    {
        ...
        if($request->features_section_bottom)
        {
            ...
            foreach($request->features_section_bottom['title'] as $key => $value){
                ...
                {
                    $image = ImageManager::upload('banner/', 'webp', $request->features_section_bottom_icon[$key], 'def.jpg');
                } else {
                    $image = '';
                }
    ...
    public function update_seller_settings(Request $request)
    {
        ...
        $request->validate([
            ...
            'minimum_withdraw_duration' => 'required|min:0',
            'minimum_withdraw_value' => 'required|min:0',
        ]);
        ...
        // minimum_withdraw_duration
        BusinessSetting::updateOrInsert(['type' => 'minimum_withdraw_duration'], [
            'value' => $request->minimum_withdraw_duration ?? 0,
            'updated_at' => now()
        ]);

        // minimum_withdraw_value
        BusinessSetting::updateOrInsert(['type' => 'minimum_withdraw_value'], [
            'value' => $request->minimum_withdraw_value ?? 0,
            'updated_at' => now()
        ]);
        
        // automatic_balance_request
        BusinessSetting::updateOrInsert(['type' => 'automatic_balance_request'], [
            'value' => $request->automatic_balance_request ?? 0,
            'updated_at' => now()
        ]);

        // automatic_balance_request_duration
        BusinessSetting::updateOrInsert(['type' => 'automatic_balance_request_duration'], [
            'value' => $request->automatic_balance_request_duration ?? 0,
            'updated_at' => now()
        ]);
    ...
    public function company_reliability_store(Request $request){
        ...
        {
            $image = ImageManager::upload('company-reliability/', 'webp', $request->file('image'), 'def.jpg');
        } else {
            $image = '';
        }
