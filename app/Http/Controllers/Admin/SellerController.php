...
use App\Mail\ApprovalSellerMail;
...
class SellerController extends Controller
{
    ...
    public function updateStatus(Request $request)
    {
        ...
        if ($request->status == "approved") {
            ...
            Mail::to($seller->email)->send(new ApprovalSellerMail($seller->id));
        } else if ($request->status == "rejected") {
            Toastr::info(translate('Seller_has_been_rejected_successfully'));
            Mail::to($seller->email)->send(new ApprovalWaitingMail(translate('Seller_has_been_rejected_successfully')));
        } else if ($request->status == "suspended") {
            $seller->auth_token = Str::random(80);
            Mail::to($seller->email)->send(new ApprovalWaitingMail(translate('Seller_has_been_suspended_successfully')));
