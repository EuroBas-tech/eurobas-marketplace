...
class DeliveryManController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($request->file('identity_image'))) {
            foreach ($request->identity_image as $img) {
                array_push($id_img_names, ImageManager::upload('delivery-man/', 'webp', $img, 'def.jpg'));
            }
        ...
        $dm->image = ImageManager::upload('delivery-man/', 'webp', $request->file('image'), 'def.jpg');
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!empty($request->file('identity_image'))) {
            ...
            $img_keeper = [];
            foreach ($request->identity_image as $img) {
                array_push($img_keeper, ImageManager::upload('delivery-man/', 'webp', $img, 'def.jpg'));
            }
    ...
    public function export(Request $request){
                ...
                if (!empty($key)) {
                    ...
                    $query->where(function ($subquery) use ($keywords) {
                        foreach ($keywords as $keyword) {
                            $subquery->orWhere('f_name', 'like', "%{$keyword}%")
        ...
        $active = $delivery_men->where('is_active',1)->count();
        $inactive = $delivery_men->where('is_active',0)->count();
        $data = [
        ...
        return Excel::download(new DeliveryManListExport($data), 'Delivery-Man-List.xlsx');
    }

    /** order history log and earn export */
    public function order_history_log_export(Request $request,$id){
        $key = $request->search;
        $delivery_man = DeliveryMan::with('wallet')->find($id);
        $orders = Order::select('id', 'deliveryman_charge', 'order_status', 'delivery_man_id','payment_status','created_at')
            ->with('details')
            ->where(['delivery_man_id' => $id])
            ->whereHas('delivery_man', function($query){
                $query->where('seller_id',0);
        ...
        $excel_name = $request->type == 'earn' ? 'Delivery-Man-Earnings-List' : 'Delivery-Man-Order-List' ;
        return Excel::download(new DeliveryManOrderHistory($data), $excel_name.'.xlsx');
    }
}
