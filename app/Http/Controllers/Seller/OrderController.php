...
//        return view('seller-views.order.invoice',compact('order', 'seller', 'company_phone', 'company_name', 'company_email', 'company_web_logo'));
      $mpdf_view = \View::make('seller-views.order.shipping', compact('order', 'seller', 'company_phone', 'company_email', 'company_name', 'company_web_logo'));
        Helpers::gen_mpdf($mpdf_view, 'order_shipping_', $order->id);
    }
    ...
    public function status(Request $request)
    {
        ...
        $is_shipping_data_completed = $order->delivery_service_name != null && $order->third_party_delivery_tracking_id != null;

        if($request->order_status=='delivered' && $order->payment_status !='paid'){
            return response()->json(['payment_status'=>0],200);
        }
        
        if($request->order_status=='delivered' && !$is_shipping_data_completed){
            return response()->json(['shipping_data'=>0],200);
        }
        
        if($request->order_status=='shipped' && !$is_shipping_data_completed){
            return response()->json(['shipping_data'=>0],200);
        }
    ...
    public function update_deliver_info(Request $request)
    {
        $request->validate([
            ...
            'third_party_tracking_number' => 'required',
        ]);
        
        $create_track = (new ShippingTracking)->create_tracker(
            $request->third_party_tracking_number,
            $request->delivery_service_name
        );
        
        
        if($create_track['status'] == 'error') {            
            Toastr::error(\App\CPU\translate('please_check_your_tracking_number_and_courier_code_and_try_again'));
            return back();
        }
        
        if (isset($create_track['details']['data']['tracker']) && $create_track['details']['data']['tracker']['isTracked'] == true) {
            $order = Order::find($request->order_id);
            $order->delivery_type = 'third_party_delivery';
            $order->delivery_service_name = $request->delivery_service_name;
            $order->third_party_tracking_number = $request->third_party_tracking_number;
            $order->third_party_delivery_tracking_id = $create_track['details']['data']['tracker']['trackerId'];
        ...
        Toastr::success(\App\CPU\translate('check_your_shipping_informations_again!'));
        return back();
