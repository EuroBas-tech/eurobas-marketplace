...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
            ...
            foreach ($request->colors as $color) {
                ...
                if ($request->file($img)) {
                    ...
                    $request->validate([
                        ...
                        "$img.max" => translate('image_size_cant_be_greater_than_1mb'),
                    ]);
        ...
        if ($request->has('colors_active') && $request->has('colors') && count($request->colors) > 0) {
            foreach ($request->colors as $color) {
                ...
                $img = 'color_image_'.$color_;
                if($request->file($img)){
                    $image_name = ImageManager::upload('product/', 'webp', $request->file($img), 'def.jpg');
        ...
        if ($request->ajax()) {
            ...
            if ($request->file('images')) {
                foreach ($request->file('images') as $img) {
                    $image_name = ImageManager::upload('product/', 'webp', $img, 'def.jpg');
                    $product_images[] = $image_name;
            ...
            $product->thumbnail = ImageManager::upload('product/thumbnail/', 'webp', $request->file('image'), 'def.jpg');
            ...
            $product->meta_image = ImageManager::upload('product/meta/', 'webp', $request->meta_image, 'def.jpg');


        if($request->countries!=[])
        {
            $product->shipping_country = json_encode($request->countries);
        }
        else
        $product->shipping_country="All";
            $product->save();
    ...
    public function edit($id)
    {
        ...
        $shipping_country = json_decode($product->shipping_country);
        $br = Brand::orderBY('name', 'ASC')->get();
        $brand_setting = BusinessSetting::where('type', 'product_brand')->first()->value;
        $digital_product_setting = BusinessSetting::where('type', 'digital_product')->first()->value;

        return view('seller-views.product.edit', compact('categories', 'br', 'product', 'product_category', 'brand_setting', 'digital_product_setting','shipping_country'));
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->has('images') && $product->images && count(json_decode($product->images, true)) >= 8) {
            Toastr::error(translate('maximum_product_additional_images_is_8_images'));
            return back();
        }
