...
class UserProfileController extends Controller
{
    ...
    public function user_profile(Request $request)
    {
        ...
        $wishlists = $this->wishlist->whereHas('wishlistAd', function ($q) {
            return $q;
        })->where('customer_id', auth('customer')->id())->count();

        $user_ads_count = Ad::active()->where('user_id', auth('customer')->id())->count();
        ...
        return view(VIEW_FILE_NAMES['user_profile'], compact('customer_detail', 'addresses', 'wishlists', 'user_ads_count', 'total_order', 'total_loyalty_point', 'total_wallet_balance'));
    }
    ...
    public function user_account(Request $request)
    {
       ...
       return view(VIEW_FILE_NAMES['user_account'], compact('customerDetail', 'ad_images_size'));

    }

    public function user_ads() {

        $user_ads = Ad::active()->where('user_id', auth('customer')->user()->id)
        ->orderBy('created_at', 'desc')
        ->paginate(15);
      
        return view(VIEW_FILE_NAMES['account_ads'], compact('user_ads'));
    }
    ...
    public function user_update(Request $request)
    {
                ...
                'password' => [
                    ...
                    function ($attribute, $value, $fail) {
                        if (!preg_match('/[a-z]/', $value)) {
                            $fail(translate('the_password_must_contain_at_least_one_lowercase_letter'));
                        }
                        if (!preg_match('/[A-Z]/', $value)) {
                            $fail(translate('the_password_must_contain_at_least_one_uppercase_letter'));
                        }
        ...
        $user->show_phone_number = $request['show_phone_number'] && $request['show_phone_number'] == 'on' ? 1 : 0;
        $user->email = $request->email;
        $user->show_email_address = $request['show_email_address'] && $request['show_email_address'] == 'on' ? 1 : 0;
        ...
        $user->show_location_data = $request['show_location_data'] && $request['show_location_data'] == 'on' ? 1 : 0;
        ...
        if($request->hasFile('image')) {
            $user->image = ImageManager::upload('profile/images/', 'webp', $request->file('image'), 'def-image.jpg');
        }

        if($request->hasFile('cover_image')) {
            $user->cover_image = ImageManager::upload('profile/covers/', 'webp', $request->file('cover_image'), 'def-cover-image.jpg');
        }

        $user->save();

        Toastr::success(translate('user_data_updated_successfully'));
    ...
    public function edit_location_data()
    {
        ...
        return view(VIEW_FILE_NAMES['account_location_edit'], compact('countries', 'zip_restrict_status', 
        'zip_codes', 'default_location', 'user_data'));
    }

    public function show_profile(Request $request, $id, $name)
    {

        // Find user by first and last name
        $user_profile = User::with('ads')->where('id', $id)
        ->where('name', $name)
        ->first();
        ...
        $user_brands = $user_profile->ads->pluck('brand_id')->unique();
        $user_brands = Brand::whereIn('id', $user_brands)->select('id', 'name', 'image')->get();
    ...
    public function account_order_details(Request $request)
    {
        ...
        $colors = \App\Model\Color::select('name')->orderBy('name', 'asc')->pluck('name')->toArray(); 

        $refund_day_limit = \App\CPU\Helpers::get_business_settings('refund_day_limit');
        $current_date = \Carbon\Carbon::now();
        if($order){
            return view(VIEW_FILE_NAMES['account_order_details'], compact('order', 'refund_day_limit', 'current_date', 'colors'));
        }
    ...
    {
        if (auth('customer')->check()) {
            $supportTickets = SupportTicket::with(['customer:id,name,image'])
    ...
    public function ticket_submit(Request $request)
    {
        ...
        if ($request->file('image')) {
            ...
            foreach ($request->image as $key=>$value) {
                $image_name = ImageManager::upload('support-ticket/', 'webp', $value, 'def.jpg');
                $image[] = $image_name;
            }
        }
    ...
    {
        $ticket = SupportTicket::with(['customer:id,name,image', 'conversations'=>function($query){
    ...
    public function store_refund(Request $request)
    {
        ...
        if ($request->file('images')) {
            ...
            foreach ($request->file('images') as $img) {
                $product_images[] = ImageManager::upload('refund/', 'webp', $img, 'def.jpg');
            }
        ...
        $seller = Seller::with('wallet')->find($order->seller_id);
        ...
        $seller_prefered_language = $seller->prefered_language ?? 'en';

        session(['seller_prefered_language'=> $seller_prefered_language]);
        
        Mail::to($seller->email)
        ->send(new \App\Mail\RefundRequestMail($seller->id));
    ...
    public function refund_details($id)
    {
        ...
        $refund_address = $order->seller->shop->refund_address;

        if($product) {
            return view(VIEW_FILE_NAMES['refund_details'], compact('order_details', 'refund', 'refund_address','product', 'order'));
        }
