...
class WebController extends Controller
{
    ...
    public function delivery_confirmed(Request $request){
        ...
        CommonTrait::add_order_status_history($request->id, $order->seller_id , 'delivery_confirmed', 'customer');

        Toastr::success(translate('Your order is confirmed successfully'));
        return back();
    ...
    public function viewWishlist(Request $request)
    {
        ...
        $wishlists = Wishlist::with('wishlistAd')
        ->whereHas('wishlistAd', function ($q) use ($request) {
    ...
    public function storeWishlist(Request $request)
    {
        if ($request->ajax()) {
            if (auth('customer')->check()) {
                $wishlist = Wishlist::where('customer_id', auth('customer')->id())->where('ad_id', $request->ad_id)->first();
                if ($wishlist) {
                    $wishlist->delete();

                    $countWishlist = Wishlist::whereHas('wishlistAd',function($q){
                        return $q;
                    })->where('customer_id', auth('customer')->id())->count();
                    $ad_count = Wishlist::where(['ad_id' => $request->ad_id])->count();
                    session()->put('wish_list', Wishlist::where('ad_id', auth('customer')->user()->id)->pluck('ad_id')->toArray());
                    ...
                    $ad_count = Wishlist::where(['ad_id' => $request->ad_id])->count();
                    session()->put('wish_list', Wishlist::where('customer_id', auth('customer')->user()->id)->pluck('ad_id')->toArray());

                    return response()->json([
                        'success' => translate("Ad has been added to wishlist"),
    ...
    {
        $this->wishlist->where(['ad_id' => $request['id'], 'customer_id' => auth('customer')->id()])->delete();
        $data = translate('ad_has_been_remove_from_wishlist');
        $wishlists = $this->wishlist->where('customer_id', auth('customer')->id())->paginate(15);
        $brand_setting = BusinessSetting::where('type', 'product_brand')->first()->value;
        session()->put('wish_list', $this->wishlist->where('customer_id', auth('customer')->user()->id)->pluck('ad_id')->toArray());
        return response()->json([
            'success' => $data,
