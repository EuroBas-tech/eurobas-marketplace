    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('fileUpload')) {
            foreach ($request->file('fileUpload') as $image) {
                array_push($image_array, ImageManager::upload('review/', 'webp', $image, 'def.jpg'));
            }
        }

        $review = Review::where(['customer_id' => auth('customer')->id(), 'product_id' => $request['product_id']])->first();

        if ($review && $review->attachment && $request->has('fileUpload')) {
            foreach (json_decode($review->attachment) as $image) {
                ImageManager::delete('/review/' . $image);
            }
        }
        ...
        Review::updateOrCreate(
            ...
            [
                ...
                'attachment' => $request->has('fileUpload') ? json_encode($image_array) : ($review->attachment ?? null),
            ]
        );
