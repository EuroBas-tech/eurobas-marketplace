...
class RegisterController extends Controller
{
    ...
    {
        return view('theme-views.customer-views.auth.register-type');
    }

    public function register(Request $request)
    {
        if(!$request->has('type')) {
            return redirect()->route('customer.auth.sign-up-type');
        }

        if($request->type == 'company' || $request->type == 'individual') {
            $view_file_name = "register-$request->type-account";
            return view('theme-views.customer-views.auth.'.$view_file_name);
        }
    ...
    public function submit(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'agree.required' => translate('to_join_the_platform_you_must_agree_to_the_terms_and_conditions'),
        ]);
