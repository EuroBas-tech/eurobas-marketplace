    ...
    public function customer_payment_request(Request $request)
    {
        ...
        if ($currency_model == 'multi_currency') {
            ...
            if($request->payment_request_from=='app'){
                ...
                $currency_rate = 1; // القيمة الافتراضية للدولار

                if ($currency_code != 'USD') {
                    $currency = Currency::where('code', $currency_code)->first();

                    if ($currency_code == 'EUR') {
                        // معدل تحويل اليورو مقابل الدولار
                        $usd_to_eur_rate = Currency::where('code', 'USD')->first()->exchange_rate;
                        $currency_rate = 1/$usd_to_eur_rate;
                    } else {
                        // باقي العملات تحويل من الدولار إلى العملة المختارة
                        $usd_to_selected_currency_rate = $currency->exchange_rate;
                        $currency_rate =1/Currency::where('code', 'USD')->first()->exchange_rate* $usd_to_selected_currency_rate;
                    }
                }
            ...
            {
                 $currency_rate=Currency::where(['code'=>session('currency_code')])->first();
        ...
        else
        {
            if (session()->has('usd')) {
                ...
                $usdd = Currency::where(['code' => 'USD'])->first()->exchange_rate;
                session()->put('usd', $usdd);
            }
            $my_currency = \session('currency_exchange_rate');
            $rate = $my_currency / $usdd;
        }
        ...
        $payment_info = new PaymentInfo(
            ...
            payment_amount: number_format($rate * $payment_amount, 2, '.', ''),
