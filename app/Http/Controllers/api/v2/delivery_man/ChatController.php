...
class ChatController extends Controller
{
    ...
    public function send_message(Request $request, $type)
    {
        ...
        if ($type == 'seller') {
            ...
            Helpers::chatting_notification('message_from_delivery_man','seller',$seller,$delivery_man);
        } elseif ($type == 'customer') {
            $chatting->user_id          = $request->id;
            $chatting->seen_by_customer = 0;

            $customer = User::find($request->id);
            Helpers::chatting_notification('message_from_delivery_man','customer',$customer,$delivery_man);
        } elseif ($type == 'admin') {
            $chatting->admin_id         = 0;
