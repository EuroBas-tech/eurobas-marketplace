  ...
  public static function cart_to_db($request =null)
{
    ...
    if (!$user || $user == 'offline' || !isset($user->id)) {
        return;
    }

    if (session()->has('guest_id') || optional($request)->guest_id) {

        $guest_id = session('guest_id') ?? optional($request)->guest_id;
        ...
        foreach ($carts as $cart) {
            ...
            $cart->cart_group_id = isset($db_cart)
                ? $db_cart['cart_group_id']
                : str_replace('guest', $user->id, $cart['cart_group_id']);
    ...
    public static function add_to_cart($request, $from_api = false)
    {
        ...
        if(session('country_shipping') && session('country_shipping') != 'All' 
        && $product->shipping_country != 'All'
        && !in_array(session('country_shipping'), json_decode($product->shipping_country))) {
            return [
                'status' => 0,
                'message' => translate('sorry_this_product_not_shipped_to_your_country_you_can_send_message_to_the_seller')
            ];
        }
