...
class OrderManager
{
    ...
    public static function order_summary($order)
    {
        ...
        foreach ($order->details as $key => $detail) {
            ...
            $added_tax += $detail->tax_model == 'exclude' ? $detail->tax : 0;
    ...
    public static function wallet_manage_on_order_status_change($order, $received_by)
    {
        ...
        $added_tax = $order_summary['added_tax'];
        ...
        \Log::debug('order commission : '.$commission);
        ...
        $seller_wallet = SellerWallet::where('seller_id', $order['seller_id'])->get()->first();
        $admin_wallet = AdminWallet::where('admin_id', 1)->get()->first();

        $seller_wallet_action = new SellerWalletAction;
        $admin_wallet_action = new AdminWalletAction;

        $admin_wallet_action->admin_wallet_id = $admin_wallet->id;

        $seller_wallet_action->seller_id = $order['seller_id'];
        $seller_wallet_action->order_id = $order['id'];
        $admin_wallet_action->order_id = $order['id'];
        $seller_wallet_action->seller_wallet_id = $seller_wallet->id;

        $seller_wallet_action->save();
        $admin_wallet_action->save();

        $is_added_pending_withdraw = false;
        $is_added_pending_amount = false;

        $is_added_seller_delivery_charge_earned = false;

        $is_added_seller_total_tax_collected = false;
        $is_added_admin_total_tax_collected = false;

        $is_added_seller_commission_given = false;

        $is_added_seller_collected_cash = false;
        $is_added_admin_collected_cash = false;

        $seller_payout_history = new SellerPayoutHistory;

        $seller_payout_history->old_pending_withdraw = $seller_wallet->pending_withdraw;

        $seller_payout_history->old_withdrawn = $seller_wallet->withdrawn;

        $seller_payout_history->old_total_earning = $seller_wallet->total_earning;
        ...
        if ($order->coupon_code && $order->coupon_code != '0' && $order->seller_is == 'seller' && $order->discount_type == 'coupon_discount') {
            if ($order->coupon_discount_bearer == 'inhouse') {
                ...
                if(!$is_added_pending_withdraw) {
                    $seller_wallet->pending_withdraw += $order->discount_amount;
                    $seller_wallet_action->pending_withdraw = $order->discount_amount;
                    $is_added_pending_withdraw = true;
                }

                if(!$is_added_pending_amount) {
                    $admin_wallet->pending_amount += $order->discount_amount;
                    $admin_wallet_action->pending_amount = $order->discount_amount;
                    $is_added_pending_amount = true;
        ...
        if($order->is_shipping_free && $order->seller_is == 'seller') {
            ...
            if(!$is_added_pending_withdraw) {
                $seller_wallet->pending_withdraw += $order->extra_discount;
                $seller_wallet_action->pending_withdraw = $order->extra_discount;
                $is_added_pending_withdraw = true;
            }

            if(!$is_added_pending_amount) {
                $admin_wallet->pending_amount += $order->extra_discount;
                $admin_wallet_action->pending_amount = $order->extra_discount;
                $is_added_pending_amount = true;
            }
        ...
        if (!$order['is_shipping_free'] && $order->seller_is == 'seller') {
            if(!$is_added_seller_delivery_charge_earned) {
                $seller_wallet->delivery_charge_earned += $order['shipping_cost'];
                $seller_wallet_action->delivery_charge_earned = $order['shipping_cost'];
                $is_added_seller_delivery_charge_earned = true;
            }
        }
        ...
        if ($order['payment_method'] == 'cash_on_delivery' || $order['payment_method'] == 'offline_payment') {
            DB::table('order_transactions')->insert([
                ...
                'seller_amount' => $order['order_amount'] - $commission,
            ...
            $admin_wallet_action->commission_earned = $commission;
            $wallet->save();
            $admin_wallet_action->save();

            if ($order['seller_is'] == 'admin') {
                $wallet = AdminWallet::where('admin_id', 1)->first();
                $wallet->inhouse_earning += $order['order_amount'];
                if(!$is_added_admin_total_tax_collected) {
                    $admin_wallet->total_tax_collected += $order_summary['total_tax'];
                    $admin_wallet_action->total_tax_collected = $order_summary['total_tax'];
                    $is_added_admin_total_tax_collected = true;
                }
                $admin_wallet->save();
                $admin_wallet_action->save();
            } else {
                if(!$is_added_seller_commission_given) {
                    $seller_wallet->commission_given += $commission;
                    $seller_wallet_action->commission_given = $commission;
                    $is_added_seller_commission_given = true;
                }
                
                if(!$is_added_seller_total_tax_collected) {
                    $seller_wallet_action->total_tax_collected = $order_summary['total_tax'];
                    $seller_wallet->total_tax_collected += $order_summary['total_tax'];
                    $is_added_seller_total_tax_collected = true;
                }
                ...
                if(!$is_added_seller_collected_cash) {
                    $seller_wallet->collected_cash += $order['order_amount']; // Total order amount
                    $seller_wallet_action->collected_cash = $order['order_amount']; // Total order amount
                    $is_added_seller_collected_cash = true;
                }
                
                if(!$is_added_admin_collected_cash) {
                    $admin_wallet->collected_cash += $order['order_amount']; // Total order amount
                    $admin_wallet_action->collected_cash = $order['order_amount']; // Total order amount
                    $is_added_admin_collected_cash = true;
                }

                if(!$is_added_pending_withdraw) {
                    $seller_wallet->pending_withdraw += ($order['order_amount'] - $commission);
                    $seller_wallet_action->pending_withdraw = ($order['order_amount'] - $commission);
                    $is_added_pending_withdraw = true;
                }

                if(!$is_added_pending_amount) {
                    $admin_wallet->pending_amount += ($order['order_amount'] - $commission);
                    $admin_wallet_action->pending_amount = ($order['order_amount'] - $commission);
                    $is_added_pending_amount = true;
                }
            ...
            // Admin wallet logs delivery charge for revision
            $wallet->save();
            $admin_wallet_action->save();
        ...
        $seller_wallet_action_history->seller_wallet_id = $seller_wallet->id;
        $seller_wallet_action_history->current_pending_withdraw = $seller_wallet->pending_withdraw;
        $seller_wallet_action_history->current_total_earning = $seller_wallet->total_earning;
        $seller_wallet_action_history->current_withdrawn = $seller_wallet->withdrawn;
        
        $seller_wallet_action_history->current_suspended_amount = $seller_wallet?->walletActions
        ...
        $seller_payout_history->amount = $seller_wallet_action->pending_withdraw;
        $seller_payout_history->payment_method = null;
        $seller_payout_history->seller_id = $order['seller_id'];
        $seller_payout_history->new_pending_withdraw = $seller_wallet->pending_withdraw;
        $seller_payout_history->new_withdrawn = $seller_wallet->withdrawn;
        $seller_payout_history->new_total_earning = $seller_wallet->total_earning;
    ...
    public static function generate_order($data)
    {
        ...
        print_r('admin_commission' . $admin_commission);
        $is_shipping_free = 0;
        ...
        self::add_order_status_history($order_id, $customer_id, 'pending', 'customer');
    ...
    public static function order_insert($order_data) : int
    {
        ...
        self::add_order_status_history($order_data['order_id'], auth('customer')->id(), 'pending', 'customer');
